...
class IPModule:
    ...
    def compile_pybind11(self):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to compile pybind wrapper for {self.lib_name}!")
    ...
    def compile_shared_lib(self):
        ...
        for src in srcs:
            subcmd = cmd
            subcmd += " " + src
            obj = f"/tmp/allo/{src.split('/')[-1]}.o"
            subcmd += " -o " + obj
            print(subcmd)
            try:
                subprocess.check_output(subcmd, shell=True)
            except subprocess.CalledProcessError:
                raise RuntimeError(f"Failed to compile {src.split('/')[-1]}.o!")
            obj_files.append(obj)
        cmd = f"g++ -shared -o /tmp/allo/lib{self.top}.so " + " ".join(obj_files)
        print(cmd)
        try:
            subprocess.check_output(cmd, shell=True)
        except subprocess.CalledProcessError:
            raise RuntimeError(f"Failed to compile {self.top}.so!")
        return f"/tmp/allo/lib{self.top}.so"
