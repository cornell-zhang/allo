...
def generate_diagnostic_report(
    errors: list[ValidationError], mlir_text: str, context_radius: int = 3
) -> str:
    ...
    for err in errors:
        for i in range(
            max(1, err.line_num - context_radius),
...
def write_diagnostic_file(
    errors: list[ValidationError], mlir_text: str, project: str
) -> str:
...
def format_error_summary(errors: list[ValidationError], project: str = None) -> str:
    ...
    for err_type, err_list in error_by_type.items():
        ...
        if len(err_list) == 1:
            error_summary.append(
                f"  - {err_type} at line {sample.line_num}: {sample.message}"
            )
        else:
            error_summary.append(
                f"  - {err_type}: {len(err_list)} occurrences (first at line {sample.line_num})"
            )
