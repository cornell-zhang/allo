...
# pylint: disable=no-name-in-module,no-member,too-many-locals,too-many-arguments
...
# Fixed point struct modeled via Fixed Integer type.
...
def _extract_body(code, func_name):
    ...
    return code[start:pos - 1] if depth == 0 else f"// Unmatched braces in {func_name}\n"
...
def _reindent(body, indent="    "):
    ...
    return "\n".join("" if not ln.strip() else indent + ln[min_indent:] for ln in lines)


class _MemInfo:  # pylint: disable=too-few-public-methods
    """Simple container for memory info."""
    def __init__(self, name, elem_type, size, dims):
        self.name = name
...
def _parse_memory_comments(body):
    """Parse __xls_memory_decl__ and __xls_state_vars__ comments from body.
    Returns (decl_strings, state_strings, cleaned_body, mem_info_list).
    """
    mem_re = re.compile(r"^\s*//\s*__xls_memory_decl__:\s*(.+)$")
    state_re = re.compile(r"^\s*//\s*__xls_state_vars__:\s*(.+)$")

    decls, states, clean_lines, mems = [], [], [], []
    ...
    for line in body.split("\n"):
        ...
        if mem_m:
            parts = [p.split(";")[0].split("//")[0].strip() for p in mem_m.group(1).split(",")]
            if len(parts) >= 3:
                ...
                if size_m:
                    ...
                    mems.append(_MemInfo(name, elem_type, size, int_dims or [size]))
        elif state_m:
...
def _gen_textproto(mems):
  ...
  from_config {{ kind: RAM_ABSTRACT depth: {m.size} }}
  to_config {{ kind: RAM_1R1W depth: {m.size} }}
  from_channels_logical_to_physical: {{ key: "abstract_read_req" value: "{m.name}_read_request" }}
  from_channels_logical_to_physical: {{ key: "abstract_read_resp" value: "{m.name}_read_response" }}
  from_channels_logical_to_physical: {{ key: "abstract_write_req" value: "{m.name}_write_request" }}
  from_channels_logical_to_physical: {{ key: "write_completion" value: "{m.name}_write_response" }}
  to_name_prefix: "{m.name}_"
}}
""")
    return "\n".join(lines)
...
def _wrap_xlscc(core_code, top_name, func_name, inputs, use_memory):
    """Wrap core C++ code in TestBlock class with channels."""
    # No array inputs = combinational, return as-is
    if not inputs:
        return core_code, ""
...
# Main XLS Module class
class XLSCCModule:  # pylint: disable=too-many-instance-attributes
    """XLS backend module: generates XLS[cc] C++ from MLIR."""

    def __init__(self, mlir_text_or_module, top_func_name, project=None, use_memory=False):
