...
func.func @softmax(%A: memref<2x16x32xf32>) -> memref<2x16x32xf32> {
      ...
      ["parallel", "parallel", "parallel"], name = "exp"} ins(%A, %0 : memref<2x16x32xf32>, memref<2x16xf32>)
      outs(%B : memref<2x16x32xf32>) {
          ...
          linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>], iterator_types = ["parallel",
      "parallel", "reduction"], name = "add"} ins(%B : memref<2x16x32xf32>) outs(%1 : memref<2x16xf32>) {
          ^bb0(%IN: f32, %OUT: f32):
            %7 = arith.addf %IN, %OUT : f32
            linalg.yield %7 : f32
          }
          linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types =
      ["parallel", "parallel", "parallel"], name = "div"} ins(%B, %1 : memref<2x16x32xf32>, memref<2x16xf32>)
      outs(%B : memref<2x16x32xf32>) {
          ^bb0(%IN1: f32, %IN2: f32, %OUT: f32):
