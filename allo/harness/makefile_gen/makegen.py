...
def mk_run(target, data, desc_file, path, platform):
    ...
    makefile_lst = [
        "makegen_us_alveo.py",
        "makegen_versal_alveo.py",
        "makegen_versal_ps.py",
        "makegen_zynqmp.py",
    ]
    ...
    if "zc" in blocklist:
        if "makegen_zynqmp.py" in makefile_lst:
            makefile_lst.remove("makegen_zynqmp.py")
    if "vck" in blocklist:
        if "makegen_versal_ps.py" in makefile_lst:
            makefile_lst.remove("makegen_versal_ps.py")
        makefile_lst.remove("makegen_versal_alveo.py")
    if "dma" in blocklist:
        if "makegen_versal_alveo.py" in makefile_lst:
            makefile_lst.remove("makegen_versal_alveo.py")
...
def report_gen(target, data):
    ...
    if (
        "testinfo" in data
        and "profile" in data["testinfo"]
        and data["testinfo"]["profile"] == "no"
    ):
        pass
    else:
