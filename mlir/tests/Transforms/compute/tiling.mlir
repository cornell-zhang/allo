...
module {
    func.func @matrix_multiply(%A: memref<1024x512xf32>, %B: memref<512x1024xf32>, %C: memref<1024x1024xf32>)
    {
        ...
        %li_outer, %li_inner = allo.split (%li, 16)
        %li_in_out, %li_in_in = allo.split (%li_inner, 4) // nest with split
        %li_out_out, %li_out_in = allo.split (%li_outer, 8) // multiple split
        %lj_out, %lj_in, %lk_out, %lk_in = allo.tile (%lj, %lk, 16, 8) // split & tile
        %l14, %l15, %l16, %l17 = allo.tile (%li_in_out, %li_in_in, 2, 2) // nest with split (failed)
        allo.unroll (%lk_in, 16) // unroll
        allo.pipeline (%lk_out, 1) // pipeline
        allo.parallel (%lj_in) // parallel
        return
    }
}