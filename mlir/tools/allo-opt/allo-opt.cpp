...
static llvm::cl::opt<bool> enableOpt("opt",
                                     llvm::cl::desc("Enable Allo schedules"),
                                     llvm::cl::init(false));
...
int runJiTCompiler(mlir::ModuleOp module) {
  ...
  bool found = mlir::allo::getEnv("LLVM_BUILD_DIR", LLVM_BUILD_DIR);
  if (!found) {
    llvm::errs() << "Error: LLVM_BUILD_DIR not found\n";
  }
  std::string Allo_DIALECT_BUILD_DIR;
  found = mlir::allo::getEnv("Allo_DIALECT_BUILD_DIR", Allo_DIALECT_BUILD_DIR);
  if (!found) {
    llvm::errs() << "Error: Allo_DIALECT_BUILD_DIR not found\n";
  }
  ...
  std::string allo_runtime_lib =
      Allo_DIALECT_BUILD_DIR + "/lib/liballo_runtime_utils.so";
...
int main(int argc, char **argv) {
  ...
  registry.insert<mlir::allo::AlloDialect>();
  ...
  if (enableOpt) {
    pm.addPass(mlir::allo::createLoopTransformationPass());
  }

  if (dataPlacement) {
    pm.addPass(mlir::allo::createDataPlacementPass());
  }

  if (memRefDCE) {
    pm.addPass(mlir::allo::createMemRefDCEPass());
  }

  if (lowerComposite) {
    pm.addPass(mlir::allo::createLowerCompositeTypePass());
  }

  if (fixedPointToInteger) {
    pm.addPass(mlir::allo::createFixedPointToIntegerPass());
  }

  // lowerPrintOps should be run after lowering fixed point to integer
  if (lowerPrintOps) {
    pm.addPass(mlir::allo::createLowerPrintOpsPass());
  }

  if (anyWidthInteger) {
    pm.addPass(mlir::allo::createAnyWidthIntegerPass());
  }

  if (moveReturnToInput) {
    pm.addPass(mlir::allo::createMoveReturnToInputPass());
  }

  if (lowerBitOps) {
    pm.addPass(mlir::allo::createLowerBitOpsPass());
  }

  if (legalizeCast) {
    pm.addPass(mlir::allo::createLegalizeCastPass());
  }

  if (removeStrideMap) {
    pm.addPass(mlir::allo::createRemoveStrideMapPass());
  }
  ...
  if (runJiT || lowerToLLVM) {
    ...
    pm.addPass(mlir::allo::createAlloToLLVMLoweringPass());
  }
