...
struct AffineValueMapBuilder {
    ...
    // since it's used to accelerate repeated failed import attempts on the same values.
    void reset();
    // add results to the final value map
    // optional if only cares about how to compose the results.
    void addResults(ArrayRef<Value> results) { llvm::append_range(this->results, results); }
