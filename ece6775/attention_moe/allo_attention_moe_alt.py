...
def optimize_attention_moe_with_composition(
    ...
    s_attn.pipeline(
        "scaled_dot_product_attention:k"
    )  # Pipeline k reduction loops (qkt_k, sv_k)
    s_attn.pipeline(
        "scaled_dot_product_attention:j"
    )  # Pipeline j loops for output dimension
    s_attn.pipeline(
        "scaled_dot_product_attention:i"
    )  # Pipeline i loops (sv_i) to overlap row processing
    print("  - Applied pipeline to i (row), k (reduction), and j (output) loops")
    print(
        "  - Note: Loop reordering (i,k,j) enables better memory access and pipelining"
    )
    ...
    print(
        "  - Created scaled_dot_product_attention schedule with pipeline optimizations"
    )
