...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(
        self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, verbose: bool = False
    ) -> torch.Tensor:
        """
    ...
    def forward(
        ...
        if verbose:
            ...
            for i in range(num_tokens):
                ...
                tokens_per_expert_count[num_experts_selected] = (
                    tokens_per_expert_count.get(num_experts_selected, 0) + 1
                )
            ...
            for expert_idx, count in sorted(expert_counts.items()):
                ...
                print(
                    f"  Expert {expert_idx}: {count} selections ({percentage:.1f}% of {total_selections} total selections)"
                )

            # Verify top-k: each token selects exactly k experts
            if tokens_per_expert_count.get(self.k, 0) == num_tokens:
                print(
                    f"[Gate Routing] ✓ All {num_tokens} tokens select exactly {self.k} expert(s)"
                )
            else:
                print(
                    f"[Gate Routing] ⚠ Expected all tokens to select {self.k} expert(s), but got: {tokens_per_expert_count}"
                )
...
class MoELayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, verbose: bool = False) -> torch.Tensor:
        ...
        gate_weights, top_k_indices = self.gate(
            x_flat, verbose=verbose
        )  # [N, num_experts], [N, k]

        # Initialize output tensor
        output = torch.zeros(
            x_flat.shape[0], self.output_dim, device=x.device, dtype=x.dtype
        )
        ...
        for expert_idx in range(self.num_experts):
            ...
            expert_weights = gate_weights[token_indices, expert_idx].unsqueeze(
                1
            )  # [num_tokens, 1]
        ...
        if verbose:
            active_experts = [
                idx for idx, count in expert_usage_stats.items() if count > 0
            ]
            ...
            if top_k_indices.shape[1] != self.k:
                print(
                    f"[MoE Verification] ✗ ERROR: Expected k={self.k}, but top_k_indices has shape {top_k_indices.shape}"
                )
            ...
            if total_usage != expected_total_usage:
                print(
                    f"[MoE Verification] ⚠ Expected {expected_total_usage} expert-token pairs (={num_tokens} tokens × {self.k} experts), but got {total_usage}"
                )

            # Check if weights are normalized (should sum to 1.0 per token)
            sample_weights = gate_weights.sum(dim=1)
            weights_normalized = torch.allclose(
                sample_weights, torch.ones_like(sample_weights), atol=1e-5
            )
            ...
            print(
                f"  ✓ Expert distribution: {dict(sorted(expert_usage_stats.items()))}"
            )
...
def verify_gate_layer(
    ...
    if imbalance_ratio < 10:
        print(
            f"  ✓ Routing is reasonably balanced (imbalance ratio: {imbalance_ratio:.2f})"
        )
    else:
    ...
    if all_normalized:
        ...
        print(
            f"    Weight sums range: [{weight_sums.min():.6f}, {weight_sums.max():.6f}]"
        )
    ...
    # Pattern B: opposite
    pattern_a = torch.cat(
        [torch.ones(input_dim // 2), -torch.ones(input_dim // 2)]
    ).unsqueeze(0)
    pattern_b = torch.cat(
        [-torch.ones(input_dim // 2), torch.ones(input_dim // 2)]
    ).unsqueeze(0)
    ...
    if a_consistent and b_consistent:
        ...
        print(
            f"  ⚠ Same patterns route to different experts (expected with random weights)"
        )
...
if __name__ == "__main__":
    ...
    gate, expert_counts = verify_gate_layer(
        num_tokens=128, input_dim=96, num_experts=2, k=1, seed=42
    )
