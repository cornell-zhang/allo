...
def print_moe_config(model_name):
    ...
    try:
        ...
        print(f"Vocabulary size: {config.vocab_size if hasattr(config, 'vocab_size') else 'N/A'}")
        ...
        if hasattr(config, 'num_experts_per_tok'):
            print(f"Number of experts per token (num_experts_per_tok): {config.num_experts_per_tok}")
        ...
        if hasattr(config, 'num_attention_heads'):
            print(f"Number of attention heads (num_attention_heads): {config.num_attention_heads}")
        ...
        if hasattr(config, 'num_hidden_layers'):
            print(f"Number of hidden layers (num_hidden_layers): {config.num_hidden_layers}")
        ...
        hidden_size = getattr(config, 'hidden_size', getattr(config, 'd_model', getattr(config, 'n_embd', None)))
        num_experts = getattr(config, 'num_local_experts', getattr(config, 'num_experts', None))
        intermediate_size = getattr(config, 'intermediate_size', getattr(config, 'ffn_dim', getattr(config, 'n_inner', None)))
        num_experts_per_tok = getattr(config, 'num_experts_per_tok', getattr(config, 'num_experts_to_select', 1))
        ...
        if num_experts_per_tok:
            print(f"k = {num_experts_per_tok}  # Top-k (number of experts activated per token)")
        print(f"batch_size = 1  # Batch size (adjustable as needed)")
        print(f"seq_len = 128  # Sequence length (adjustable as needed, actual model may support longer)")
        ...
        print("If the model requires trust_remote_code=True, make sure to trust the model")
