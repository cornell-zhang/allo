...
def moe_layer[Ty, B, L, D_in, D_out, E, K, D_hidden](
    ...
    gate_logits = linear2d[Ty, Ty, Ty, N, E, D_in](x_flat, gate_weight, gate_bias)  # [N, E]
...
def optimize_moe_with_composition(
    ...
    s_linear_fc1 = allo.customize(allo_nn.linear2d, instantiate=[Ty, Ty, Ty, N, D_hidden, D_in])
    s_gelu = allo.customize(allo_nn.GeLU, instantiate=[Ty, N, D_hidden])
    s_linear_fc2 = allo.customize(allo_nn.linear2d, instantiate=[Ty, Ty, Ty, N, D_out, D_hidden])
    ...
    s_moe = allo.customize(
        moe_layer,
        instantiate=[Ty, batch_size, seq_len, input_dim, output_dim, num_experts, k, hidden_dim]
    )
    ...
    # gate linear2d
    s_gate_linear = allo.customize(allo_nn.linear2d, instantiate=[Ty, Ty, Ty, N, E, D_in])
