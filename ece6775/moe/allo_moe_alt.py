...
def optimize_moe_schedule(
    ...
    print(
        "  - Created expert schedule with pipeline, unroll, and partition optimizations"
    )
...
if __name__ == "__main__":
    ...
    expert_fc1_weights_pt = torch.stack(
        [exp.fc1.weight.data for exp in pytorch_moe.experts]
    )
    expert_fc1_biases_pt = torch.stack(
        [exp.fc1.bias.data for exp in pytorch_moe.experts]
    )
    expert_fc2_weights_pt = torch.stack(
        [exp.fc2.weight.data for exp in pytorch_moe.experts]
    )
    expert_fc2_biases_pt = torch.stack(
        [exp.fc2.bias.data for exp in pytorch_moe.experts]
    )
