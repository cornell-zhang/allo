...
if __name__ == "__main__":
    ...
    check_x=np.array(check[0]).astype(np.float64)
    check_y=np.array(check[1]).astype(np.float64)
    check_z=np.array(check[2]).astype(np.float64)
    
   
    forceX=np.zeros(shape=(nAtoms), dtype=float)
    forceY=np.zeros(shape=(nAtoms), dtype=float)
    forceZ=np.zeros(shape=(nAtoms), dtype=float)

    position_x = np.array(input[0]).astype(np.float64)
    position_y = np.array(input[1]).astype(np.float64)
    position_z = np.array(input[2]).astype(np.float64)
    NL=np.array(input[3]).astype(np.int32)
    print("here")
    ...
    forceX=mod_x(position_x, position_y, position_z,NL)
    forceY=mod_y(position_x, position_y, position_z,NL)
    forceZ=mod_z(position_x, position_y, position_z,NL)
