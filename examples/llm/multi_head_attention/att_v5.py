...
def gemm(A: float32[L, h_d], B: float32[L, h_d]) -> float32[L, L]:
...
def test_attention():
    ...
    s5.partition(s5.Q_h, partition.Cyclic, dim=2, factor=8)
    s5.partition(s5.K_h, partition.Cyclic, dim=2, factor=8)
    s5.partition(s5.V_h, partition.Cyclic, dim=0, factor=32)
    ...
    mode = "csyn"
    print(s6.module)
    s6.build(target="vitis_hls", mode=mode, project=f"{mode}_att_partial_par_{P}_heads_{H}_dim_{D}_length_{L}.prj")()
    print(my_solution)
    print("-"*100)
    print(solution)
