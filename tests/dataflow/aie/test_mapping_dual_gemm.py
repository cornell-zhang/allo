...
def gen_pingpong_gemm_mapping_primitive(Pm, Pn, Pk, col_num=4, row_num=4):
    ...
    for i in range(Pm):
        bases.append([])
        for j in range(Pn):
            base = f"gemma_0_{i}_{j}"
            for k in range(1, Pk):
                mapping_primitives.append(("chain", [base, f"gemma_{k}_{i}_{j}"]))
                base += f"-gemma_{k}_{i}_{j}"
            bases[i].append(base)
    ...
    for i in range(Pm):
        bases.append([])
        for j in range(Pn):
            base = f"gemmb_0_{i}_{j}"
            for k in range(1, Pk):
                mapping_primitives.append(("chain", [base, f"gemmb_{k}_{i}_{j}"]))
                base += f"-gemmb_{k}_{i}_{j}"
            bases[i].append(base)
...
if __name__ == "__main__":
    ...
    # - i8
    # try:
    #     _test_pingpong_gemm(128, 128, 256, 4, 4, 8, int8, int8)
    # except:
