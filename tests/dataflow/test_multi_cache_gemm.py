...
def top():
    ...
    L3_A = df.pipe(dtype=UInt(Rt * 16), shape=(), depth=4)
    L3_B = df.pipe(dtype=UInt(Ct * 16), shape=(), depth=4)
    L3_C = df.pipe(dtype=UInt(Rt * 16), shape=(), depth=4)

    L2_A = df.array(df.pipe(dtype=UInt(Rt * 16), shape=(), depth=4), shape=(P0 - 1,))
    L2_B = df.array(df.pipe(dtype=UInt(Ct * 16), shape=(), depth=4), shape=(P1 - 1,))

    L1_C = df.array(df.pipe(dtype=UInt(Rt * 16), shape=(), depth=4), shape=(Rt, Ct))
    L2_C = df.array(df.pipe(dtype=UInt(Rt * 16), shape=(), depth=4), shape=(Ct,))

    fifo_A = df.array(df.pipe(dtype=int16, shape=(), depth=4), shape=(Rt, Ct))
    fifo_B = df.array(df.pipe(dtype=int16, shape=(), depth=4), shape=(Rt, Ct))

    @df.kernel(mapping=[1])
    def offchip_loadA(A_Packed: UInt(Rt * 16)[M * K // Rt]):
        for mt, nt in dsl.grid(M // Rt, N // Ct):
            for k in range(K):
                L3_A.put(A_Packed[mt * K + k])

    @df.kernel(mapping=[1])
    def offchip_loadB(B_Packed: UInt(Ct * 16)[K * N // Ct]):
        for mt, nt in dsl.grid(M // Rt, N // Ct):
            for k in range(K):
    ...
    @df.kernel(mapping=[1])
    def offchip_store(C_Packed: UInt(Rt * 16)[M * N // Rt]):
        for mt, nt in dsl.grid(M // Rt, N // Ct):
            for n in range(Ct):
...
def test_large_scale_gemm():
    def serialize_A(matrix_A):
        A_ser = np.zeros((M * K), dtype=np.int16)
    ...
    def serialize_B(matrix_B):
        B_ser = np.zeros((K * N), dtype=np.int16)
    ...
    def deserialize_C(C_ser):
        matrix_C = np.zeros((M, N), dtype=np.int16)
        for mt in range(M // Rt):
            for n in range(N):
                for m in range(Rt):
    ...
    # np_type_A = get_np_struct_type(Rt * 16)
    # np_type_B = get_np_struct_type(Ct * 16)
    # np_type_C = get_np_struct_type(Rt * 16)
    ...
    if hls.is_available("vitis_hls"):
        ...
        C = deserialize_C(C_packed.view(np.int16))
        print(C)
        print(C_golden)
