...
def test_stateful_scalar_hls():
    ...
    assert re.search(r"static\s+ap_int<4>", code), \
    ...
    assert re.search(static_init_pattern, code), \
...
def test_moving_average_hls():
    ...
    assert re.search(r"static\s+float\s+\w+\s*\[\s*4\s*\]", code), \
